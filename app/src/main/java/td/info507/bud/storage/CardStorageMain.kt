...
object CardStorageMain {
    ...
    fun get(context: Context): Storage<CardMainModel> {
        var storage: Storage<CardMainModel> = CardNoneStorageMain()
        when (getStorage(context)) {
            NONE -> storage = CardNoneStorageMain()
            FILE_JSON -> storage = CardJSONFileStorageMain(context)
        }
        return storage
    }
}